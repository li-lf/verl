...
class RolloutCorrectionConfig(BaseConfig):
    ...
    Args:
        ...
        log_rejected_samples (bool): Whether to log rejected samples to file.
            - True: Save rejected samples (prompt, response, IS weights, etc.) to JSON file
            - False: Skip logging rejected samples
            - Only effective when rollout_rs is enabled
            Default: False (disabled)

        log_rejected_samples_path (Optional[str]): Path to save rejected samples JSON file.
            - If None, uses default path: {checkpoint_dir}/rejected_samples_{step}.json
            - Only used when log_rejected_samples=True
            Default: None
    ...
    log_rejected_samples_path: Optional[str] = None
