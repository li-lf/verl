...
def compute_rollout_rejection_mask(
    ...
    return_rejected_info: bool = False,
) -> tuple[torch.Tensor, dict[str, float], Optional[dict]]:
    ...
    Returns:
        Tuple containing:
            ...
            rejected_info: Optional dict with rejected sample information (only if return_rejected_info=True):
                - rejected_indices: List of rejected sequence indices
                - rejected_log_ratios: List of log ratios for rejected sequences
                - rejected_is_weights: List of IS weights for rejected sequences
    """
    ...
    # Collect rejected sample information if requested
    rejected_info: Optional[dict] = None
    if return_rejected_info:
        # For sequence-level RS (sequence/geometric), identify rejected sequences
        if rollout_rs in ["sequence", "geometric"]:
            # Check first token's mask (all tokens in sequence have same mask)
            rejected_seq_mask = (mask[:, 0] == 0).cpu().numpy()
            rejected_indices = rejected_seq_mask.nonzero()[0].tolist()
            
            if len(rejected_indices) > 0:
                rejected_log_ratios = log_ratio_for_metrics[rejected_indices].cpu().tolist()
                rejected_is_weights = rollout_is_weights[rejected_indices, 0].cpu().tolist()
                
                rejected_info = {
                    "rejected_indices": rejected_indices,
                    "rejected_log_ratios": rejected_log_ratios,
                    "rejected_is_weights": rejected_is_weights,
            ...
            # Token-level: collect sequences with any rejected tokens
            seq_has_rejected = verl_F.masked_sum(1 - mask, response_mask, axis=-1) > 0
            rejected_seq_mask = seq_has_rejected.cpu().numpy()
            rejected_indices = rejected_seq_mask.nonzero()[0].tolist()
    ...
    return modified_response_mask, metrics, rejected_info
...
def compute_rollout_correction_and_rejection_mask(
    ...
    if rollout_rs is not None:
        ...
        modified_response_mask, rs_metrics, _ = compute_rollout_rejection_mask(
            log_ratio=log_ratio,
            response_mask=response_mask,
            rollout_rs=rollout_rs,
            rollout_rs_threshold=rollout_rs_threshold,
            rollout_rs_threshold_lower=rollout_rs_threshold_lower,
            return_rejected_info=False,  # Will be handled in compute_rollout_correction_and_add_to_batch
        )
...
def compute_rollout_correction_and_add_to_batch(
    ...
    log_rejected_samples = rollout_corr_config.get("log_rejected_samples", False)
    log_rejected_samples_path = rollout_corr_config.get("log_rejected_samples_path", None)
    ...
    # Log rejected samples if enabled and RS is active
    if log_rejected_samples and rollout_rs is not None:
        _log_rejected_samples(
            batch=batch,
            original_response_mask=batch.batch["response_mask"],
            modified_response_mask=modified_response_mask,
            rollout_rs=rollout_rs,
            rollout_rs_threshold=rollout_rs_threshold,
            rollout_rs_threshold_lower=rollout_rs_threshold_lower,
            log_path=log_rejected_samples_path,
        )
...
def _log_rejected_samples(
    batch: DataProto,
    original_response_mask: torch.Tensor,
    modified_response_mask: torch.Tensor,
    rollout_rs: str,
    rollout_rs_threshold: float,
    rollout_rs_threshold_lower: Optional[float],
    log_path: Optional[str],
) -> None:
    """Log rejected samples to JSON file.
    
    Args:
        batch: DataProto containing prompts, responses, and log probabilities
        original_response_mask: Original response mask before rejection
        modified_response_mask: Response mask after rejection sampling
        rollout_rs: Rejection sampling mode ("token", "sequence", "geometric")
        rollout_rs_threshold: Upper threshold used for rejection
        rollout_rs_threshold_lower: Lower threshold used for rejection
        log_path: Path to save JSON file. If None, uses default path.
    """
    # Identify rejected sequences
    # For sequence-level RS (sequence/geometric), check if entire sequence is rejected
    if rollout_rs in ["sequence", "geometric"]:
        # Check first token's mask (all tokens in sequence have same mask)
        original_mask_first = original_response_mask[:, 0].cpu().numpy()
        modified_mask_first = modified_response_mask[:, 0].cpu().numpy()
        rejected_mask = (original_mask_first == 1) & (modified_mask_first == 0)
        rejected_indices = rejected_mask.nonzero()[0].tolist()
    else:
        # Token-level: sequence is rejected if any token was rejected
        original_has_valid = original_response_mask.sum(dim=-1) > 0
        modified_has_valid = modified_response_mask.sum(dim=-1) > 0
        rejected_mask = original_has_valid & (~modified_has_valid)
        rejected_indices = rejected_mask.cpu().numpy().nonzero()[0].tolist()
    
    if len(rejected_indices) == 0:
        return  # No rejected samples
    
    # Compute log ratios for rejected samples
    old_log_prob = batch.batch["old_log_probs"]
    rollout_log_prob = batch.batch["rollout_log_probs"]
    log_ratio = old_log_prob - rollout_log_prob
    
    # Compute IS weights for rejected samples
    if rollout_rs == "geometric":
        log_ratio_mean = verl_F.masked_mean(log_ratio, original_response_mask, axis=-1)
        log_ratio_mean_safe = torch.clamp(log_ratio_mean, min=-SAFETY_BOUND, max=SAFETY_BOUND)
        is_weights = torch.exp(log_ratio_mean_safe)
        log_ratios_for_samples = log_ratio_mean
    elif rollout_rs == "sequence":
        log_ratio_sum = verl_F.masked_sum(log_ratio, original_response_mask, axis=-1)
        log_ratio_sum_safe = torch.clamp(log_ratio_sum, min=-SAFETY_BOUND, max=SAFETY_BOUND)
        is_weights = torch.exp(log_ratio_sum_safe)
        log_ratios_for_samples = log_ratio_sum
    else:  # token-level
        log_ratio_safe = torch.clamp(log_ratio, min=-SAFETY_BOUND, max=SAFETY_BOUND)
        is_weights_token = torch.exp(log_ratio_safe)
        # For token-level, use mean weight per sequence
        is_weights = verl_F.masked_mean(is_weights_token, original_response_mask, axis=-1)
        log_ratios_for_samples = verl_F.masked_mean(log_ratio, original_response_mask, axis=-1)
    
    # Prepare rejected samples data
    rejected_samples = []
    for idx in rejected_indices:
        sample_data = {
            "index": int(idx),
            "is_weight": float(is_weights[idx].item()),
            "log_ratio": float(log_ratios_for_samples[idx].item()),
        }
        
        # Add prompt and response if available
        if "prompts" in batch.batch:
            prompt_ids = batch.batch["prompts"][idx]
            sample_data["prompt_ids"] = prompt_ids.cpu().tolist()
        
        if "responses" in batch.batch:
            response_ids = batch.batch["responses"][idx]
            sample_data["response_ids"] = response_ids.cpu().tolist()
        
        if "input_ids" in batch.batch:
            input_ids = batch.batch["input_ids"][idx]
            sample_data["input_ids"] = input_ids.cpu().tolist()
        
        # Add reward scores if available
        if "token_level_scores" in batch.batch:
            scores = batch.batch["token_level_scores"][idx]
            sample_data["token_level_scores"] = scores.cpu().tolist()
        
        rejected_samples.append(sample_data)
    
    # Prepare output data
    output_data = {
        "rollout_rs_mode": rollout_rs,
        "rollout_rs_threshold": rollout_rs_threshold,
        "rollout_rs_threshold_lower": rollout_rs_threshold_lower,
        "rejected_count": len(rejected_indices),
        "total_samples": len(original_response_mask),
        "rejected_samples": rejected_samples,
    }
    
    # Determine output path
    if log_path is None:
        # Try to get checkpoint directory from batch meta_info or use current directory
        checkpoint_dir = batch.meta_info.get("checkpoint_dir", None)
        if checkpoint_dir is None:
            checkpoint_dir = os.getcwd()
        step = batch.meta_info.get("step", 0)
        log_path = os.path.join(checkpoint_dir, f"rejected_samples_step_{step}.json")
